...
| `database_error` | `PGLiteDatabaseWorker.ts:255, 275` | Database operation fails. Rate-limited to one event per distinct `operation`/`tableName`/`errorType`/`errorMessage` signature per minute | `operation` (read/write)<br/>`errorType` (permission/corruption/disk_full/lock/syntax/timeout/worker_unavailable/closed/unknown)<br/>`tableName` (`transaction` for BEGIN/COMMIT/ROLLBACK)<br/>`errorName`<br/>`errorMessage` (redacted, ≤300 chars)<br/>`sqlState`<br/>`suppressedSinceLastReport` | v0.45.25 (2025-11-14) | (pending release) Added `errorName`/`errorMessage`/`sqlState`/`suppressedSinceLastReport`, rate limiting, and `transaction` table classification after a single session emitted ~62k events on 2026-08-03 that were all `errorType: unknown` |

 ...
 *
 * The recipient's view of a feedback request, embedded in a Messaging DM or
 * room. Same card as the transcript compose widget, so a request looks the same
 * whether it is being drafted, answered, or tallied.
 *
...
// Deep paths, not the Comments barrel: the barrel drags the composer's whole
// Lexical tree into every module that only wants the thread.
...
/** What `CommentThread` needs that this surface has no business knowing. */
export interface FeedbackRespondDiscussion {
...
export interface FeedbackRequestRespondProps {
  ...
  discussion?: FeedbackRespondDiscussion;
    ...
    >
      ...
      {(discussionOpen || (request.discussion?.length ?? 0) > 0) && (
        ...
        >
          ...
          {discussion ? (
            <CommentThread
              adapter={discussion.adapter}
              capabilities={discussion.capabilities}
              context={discussion.context}
              directory={discussion.directory}
              orgId={target.orgId}
              viewerUserId={viewerUserId}
              viewerActor={discussion.viewerActor}
              resolver={discussion.resolver ?? null}
              density="compact"
              emptyLabel="No comments on this request yet."
            />
          ) : (
            <div

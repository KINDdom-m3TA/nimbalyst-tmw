...
- The macOS menu bar shows your session fleet at a glance: it names a session for a few seconds as it starts, finishes, blocks or fails, flags one that has stopped responding, and stays out of the way entirely when nothing is running. Drag it onto whichever display's menu bar you want, where it sits clear of a MacBook's camera notch. The same view reaches your iPhone as a Live Activity on the Lock Screen and in the Dynamic Island, with the sessions waiting on you ranked by how long they have been waiting; tap one to open it. The phone card stands down while you are sitting at the Mac, so you never see the same counts twice.
